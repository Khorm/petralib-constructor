...
final class SourceValueBuilder extends ValueBuilder {
    ...
    void extendedBuild(ValueLoaderModel.ValueLoaderModelBuilder builder) {
        builder.sourceName(getScenarioVariable().getSource().getName())
                .sourceId(getScenarioVariable().getSource().getId())
                .sourceVersion("0")
                .parents(sourceInVariables(getScenarioVariable().getSource().getId()).stream()
                        .flatMapToLong(entity -> LongStream.of(entity.getProducerVariable().getId())).boxed().toList())
                .sourceInputVariableModels(sourceInVariables(getScenarioVariable().getSource().getId()).stream()
